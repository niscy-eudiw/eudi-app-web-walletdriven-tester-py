...
def validate_vp_token(response_json):
    ...
    auth_request_values = session["presentation_submission"]
    ...
    if pos == -1:
        ...
        attributes_requested=session["presentation_submission"]["input_descriptor"]
        attributes_received=[]
...
def validate_certificate(mdoc):
    ...
    if certificate.issuer not in trusted_CAs:
        ...
        not_valid_after=trusted_CAs[certificate.issuer]["not_valid_after"]
        not_valid_before=trusted_CAs[certificate.issuer]["not_valid_before"]
